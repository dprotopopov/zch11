...
__global__ void gradientalgorithm(double *fx, double *x, double *g, double *fx1, double *x1, double *a, double *b, double *p, int *k, int *m, int n, double t, double e1, double e2, curandState *state, int total)
{
	...
	for (int id = threadIdx.x + blockIdx.x * blockDim.x; id < total; id += gridDim.x*blockDim.x)
	{
		for (;;)
		{
			...
			double s = 0;
			for (int i = 0; i < n; i++) s += g[n*id + i] * g[n*id + i];
			if (s < e1*e1) break;
			double l = 0;
			...
			double y = (fx[id] > fx1[id]) ? fx[id] - fx1[id] : fx1[id] - fx[id];
			if (l < e2*e2 && y < e2)
			{
				for (auto i = 0; i<n; i++) x[n*id + i] = x1[n*id + i];
				fx[id] = fx1[id];
				break;
			}
...
int main()
{
	...
	double *devA, *devB, *devG, *devX, *devX1, *devP, *devFX, *devFX1;
	double *x, *fx;
	int *devK, *devM;
	...
	gradientalgorithm <<<1, N>>>(devFX, devX, devG, devFX1, devX1, devA, devB, devP, devK, devM, n, t, e1, e2, devStates, total);
